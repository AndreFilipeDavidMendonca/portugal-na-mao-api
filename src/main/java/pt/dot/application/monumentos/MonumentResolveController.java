// src/main/java/pt/dot/application/monumentos/MonumentResolveController.java
...
public class MonumentResolveController {
    ...
    public ResponseEntity<MonumentDto> resolve(
        ...
        log.info("[MonumentResolveController] Resolving monument. name='{}', lat={}, lon={}", name, lat, lon);

        MonumentDto bestMatch = wfsService.findBestMatch(name, lat, lon);

        if (bestMatch == null) {
            log.info("[MonumentResolveController] No bestMatch found for name='{}', lat={}, lon={}", name, lat, lon);
            return ResponseEntity.notFound().build();
        }

        log.info("[MonumentResolveController] Resolved monument id={} | originalName='{}'",
