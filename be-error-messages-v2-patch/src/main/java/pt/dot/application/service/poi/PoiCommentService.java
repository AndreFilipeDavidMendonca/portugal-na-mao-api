...
public class PoiCommentService {
    ...
    public PoiCommentDto add(long poiId, CreatePoiCommentRequest req) {
        ...
        if (body.isBlank()) {
            throw new ResponseStatusException(BAD_REQUEST, "Não foi possível publicar o comentário porque o texto está vazio.");
        }
    ...
    public void delete(long commentId) {
        ...
        PoiComment c = commentRepo.findById(commentId)
                .orElseThrow(() -> new ResponseStatusException(NOT_FOUND, "Não foi possível encontrar este comentário."));
        ...
        if (!isOwner && !isAdmin) {
            throw new ResponseStatusException(FORBIDDEN, "Não tens permissão para remover este comentário.");
        }
