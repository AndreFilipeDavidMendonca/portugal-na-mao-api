...
public class ChatService {
    ...
    public UUID getOrCreateConversation(UUID currentUserId, UUID friendUserId) {
        if (currentUserId.equals(friendUserId)) {
            throw Errors.badRequest("SELF_CHAT_NOT_ALLOWED", "Não podes iniciar uma conversa contigo próprio.");
        }

        Friendship friendship = friendshipRepository.findBetweenUsers(currentUserId, friendUserId)
                .orElseThrow(() -> Errors.forbidden(
                        "CHAT_REQUIRES_FRIENDSHIP",
                        "Só podes iniciar conversa com utilizadores que já fazem parte da tua lista de amigos."
                ));

        if (friendship.getStatus() != FriendshipStatus.ACCEPTED) {
            throw Errors.forbidden(
                    "CHAT_REQUIRES_ACCEPTED_FRIENDSHIP",
                    "Só podes iniciar conversa com utilizadores que já fazem parte da tua lista de amigos."
            );
        }
                ...
                .orElseGet(() -> {
                    AppUser userA = appUserRepository.findById(userAId)
                            .orElseThrow(() -> Errors.notFound(
                                    "CHAT_USER_NOT_FOUND",
                                    "Não foi possível encontrar um dos utilizadores desta conversa."
                            ));
    ...
    public void sendMessage(UUID currentUserId, UUID conversationId, SendChatMessageDto dto) {
        if (dto == null) {
            throw Errors.badRequest(
                    "CHAT_MESSAGE_PAYLOAD_MISSING",
                    "Não foi possível enviar a mensagem porque os dados estão incompletos."
            );
        }
        ...
        if (type == ChatMessageType.TEXT) {
            ...
            poiRepository.findById(poiId)
                    .orElseThrow(() -> Errors.notFound(
                            "CHAT_POI_NOT_FOUND",
                            "Não foi possível encontrar o POI que estás a tentar partilhar."
                    ));
    ...
    private ChatMessageType resolveType(SendChatMessageDto dto) {
        ...
        if (finalType == ChatMessageType.TEXT) {
            if (!hasTextBody) {
                throw Errors.badRequest(
                        "CHAT_TEXT_EMPTY",
                        "Não foi possível enviar a mensagem porque o texto está vazio."
                );
            }
            if (hasPoiPayload) {
                throw Errors.badRequest(
                        "CHAT_TEXT_CONTAINS_POI_DATA",
                        "Não foi possível enviar a mensagem porque uma mensagem de texto não pode incluir dados de POI."
                );
            }
        ...
        if (dto.getPoiId() == null) {
            throw Errors.badRequest(
                    "CHAT_POI_ID_REQUIRED",
                    "Não foi possível partilhar o POI porque falta o identificador do local."
            );
        }
        if (!hasText(dto.getPoiName())) {
            throw Errors.badRequest(
                    "CHAT_POI_NAME_REQUIRED",
                    "Não foi possível partilhar o POI porque falta o nome do local."
            );
        }
        if (hasTextBody) {
            throw Errors.badRequest(
                    "CHAT_POI_BODY_NOT_ALLOWED",
                    "Não foi possível partilhar o POI porque este tipo de mensagem não aceita texto livre."
            );
        }
    ...
    private AppUser resolveOtherParticipant(ChatConversation conversation, UUID currentUserId) {
        ...
        throw Errors.forbidden(
                ...
                "Não tens permissão para aceder a esta conversa."
        );
    }
